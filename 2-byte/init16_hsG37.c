/* variant of init_hsG36 designed to fread/fwrite binary znt2 2 byte files */
...
main (int argc,char *argv[])
{
	...
	if(argc != 3)
		{
		printf("argc %d; syntax init_hsGenome chrNo chrLen\n");
		exit(0);
		}
	...
	for(n = 0; n < chrLen; n++)
		{
		fwrite(&nVal,sizeof(short int),1,fp);
		}
...
totestMode(void)
{
	...
	do
		{
		bbChk = fread(&bbase,sizeof(short int),1,fTmReadp);
		if(bbChk != 1 && m < 20)
			{
			...
			fwrite(&bbase,sizeof(short int),1,fTmWritep);
			linPtr++;
			if (linPtr == 59)
				{
				fprintf(fTmWritep,"\n");
